...
const METHODS = ["기출문제 풀이", "인터넷 강의", "현장 강의", "스터디 모임"];

export default function CertReviewWrite({
  cert_name,
  cert_code,
  dispatch,
}: CertReviewWriteProps) {
  ...
  const addMethod = (method: string) =>
    setMethods((methods) => [...methods, method]);
  const deleteMethod = (method: string) =>
    setMethods((methods) => methods.filter((m) => m !== method));
  ...
  const submitReview = useCallback(
    async (data: SubmitCertReviewData) => {
      if (
        data.difficulty === 0 ||
        data.num_attempts === 0 ||
        data.study_method === "" ||
  ...
  const [reviewPostingStatus, setReviewPostingStatus] =
    useState<string>("plain");
  ...
  return (
    <>
      ...
      <section className={styles.questionaire}>
        <div className={styles.question}>
          ...
          <div className={styles.star_inline}>
            <StarRateComponent
              size={30}
              disabled={false}
              updater={updateStarRate}
            />
          </div>
        </div>
        <div className={styles.question}>
          <span className={styles.question_label}>몇 회만에 합격하셨나요?</span>
          <div>
            {(() => {
              let arr: any = [];
              for (let i = 1; i <= 5; i++)
                arr = [
                  ...
                  <>
                    <input
                      ...
                      id={`trynum_${i}`}
                      hidden
                      onChange={() => updateTrynum(i)}
                    ></input>
                    <label
                      key={`trynum_${i}_lab`}
                      className={styles.trynum}
                      htmlFor={`trynum_${i}`}
                    >
                      {i === 5 ? "5회 이상" : `${i}회`}
                    </label>
        ...
        <div className={styles.question}>
          <span className={styles.question_label}>얼마나 공부하셨나요?</span>
          <div className={styles.period}>
            <div>
              <input
                className={styles.period_inp}
                type="number"
                id="year"
                onChange={(e) => setStudyYear(e.target.value)}
              />
              <label className={styles.period_label} htmlFor="year">
                년
              </label>
            </div>
            <div>
              <input
                ...
                type="number"
                id="month"
                onChange={(e) => setStudyMonth(e.target.value)}
              />
              <label className={styles.period_label} htmlFor="month">
                개월
              </label>
            </div>
            <div>
              <input
                ...
                type="number"
                id="week"
                onChange={(e) => setStudyWeek(e.target.value)}
              />
              <label className={styles.period_label} htmlFor="week">
                주
              </label>
            </div>
          </div>
          ...
          <div>
            {METHODS.map((method: string, index: number) => (
              <>
                ...
                ></input>
                <label htmlFor={`method_${index}`} className={styles.method}>
                  {method}
                </label>
              </>
            ))}
        ...
        <div className={styles.question} id={styles.bookSearch}>
          <div style={{ display: "flex" }}>
            ...
            <CustomButton
              className={styles.bookSrchBt}
              onClick={toggleBookSearch}
            >
              도서검색
            </CustomButton>
          </div>
          ...
          {bookSrchVisible ? (
            <ComponentPortal
              component={<BookSearchPortal select={selectBook} />}
            />
          ) : null}
        </div>
      </section>
      <button
        className={
          reviewPostingStatus === "posting"
            ? styles.reviewSubmit + " " + styles.posting
            : styles.reviewSubmit
        }
        onClick={() => {
          ...
          submitReview(reviewSubmitData).then((status: number) => {
            if (status === 200) {
              certReviewDataCollector
                .collectData("/get_cert_review", "POST", {
                  category: "자격증코드",
                  keyword: `${cert_code}`,
                })
                .then(() => {
      ...
      >
        {reviewPostingStatus === "posting" ? "제출중..." : "제출하기"}
      </button>
    </>
  );
}
